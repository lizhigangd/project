...
function changeTheme(themeName) {/* 更换主题 */
    ...
    var themeHrefNew = themeHref.substring(0, themeHref.inArray('themes',themeHref)) + 'themes/default/topjui.' + themeName + '.css';
    // 更换主页面风格
    ...
    if ($iframe.length > 0) {
        for (var i = 1; i < $iframe.length; i++) {
            ...
            var iframeThemeHrefNew = iframeThemeHref.substring(0, iframeThemeHref.inArray('themes',themeHref)) + 'themes/default/topjui.' + themeName + '.css';
...
function generateMenu(menuId, systemName) {
    if (menuId == "") {
        ...
        var url = "http://www.discoversimba.com/json/menu/menu_" + menuId + ".json";
        $.get(
            ...
            function (data) {
                ...
                $.each(data, function (i, e) {// 循环创建手风琴的项
                    ...
                    $.get("http://www.discoversimba.com/json/menu/menu_" + e.id + ".json", function (data) {// 循环创建树的项
                        $("#tree" + e.id).tree({
                            data: data,
                            lines: false,
                            animate: true,
                            onBeforeExpand: function (node, param) {
                                ...
                                $("#tree" + e.id).tree('options').url = "http://www.discoversimba.com/json/menu/menu_" + node.id + ".json";
                            },
